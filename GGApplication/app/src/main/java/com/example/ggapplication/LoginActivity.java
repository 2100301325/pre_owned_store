            ...
            try {
                ...
                client.newCall(request).enqueue(new Callback() {
                  ...
                  if(responseModel.getCode()==200){
                      JsonObject userData = responseModel.getData();
                      int userId = userData.get("id").getAsInt();
                      int userMoney = userData.get("money").getAsInt();

                      // 将值设置到 User 类的静态属性上
                      User.setId(userId);
                      User.setMoney(userMoney);
                      System.out.println("这个用户的id"+User.getId()+"qian"+User.getMoney());
